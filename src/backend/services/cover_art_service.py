...
from .record_service import get_record_min_service
...
def add_art_service(record_id: int, content: UploadFile, filename: str):
    record = get_record_min_service(record_id)
    path = Path(
        BASE_PATH +
        f"{record["artist"]["name"]}/{filename}")
    if not Path(BASE_PATH + record["artist"]["name"]).is_dir():
